...
def handle_private_video(e: object, url: str, ydl_opts: dict, func: Callable[[str, dict, bool], Union[dict, bool]]) -> \
        Union[dict, bool]:
    ...
    def handle_choose():
        ...
        if info:
            ...
            messagebox.showinfo('Login',
                                'Login successful! Specified browser will be used until you close this app. Browser will be asked again if you attempt to download another private video that the existing accounts in this browser cannot access.')
        else:
            messagebox.showerror('Login',
                                 'Login failed. The browser you chose does not have an account with access to this video.')
        popup.destroy()
    ...
    if str(e).endswith('Private video. Sign in if you\'ve been granted access to this video'):
        ...
        Label(popup,
              text='This video is private. Please choose the browser you use that is logged into your video platform.\nIf you are on Windows and use Chrome or Edge, please close your browser before choosing.').pack()
        v = IntVar(value=0)
...
class DownloadTask(Frame):
    def __init__(self, url: str, path: str, ydl_opts: dict, parent: Widget):
        ...
        self.progress_bar = Progressbar(self, orient=HORIZONTAL, length=100, mode='determinate', variable=self.progress,
                                        value=0)
...
def parse_format(format: dict) -> dict:
    ...
    def parse_codec(codec: str) -> str:
        mapping = {'mp4v': 'H263', 'av01': 'AV1', 'avc1': 'H264/AVC', 'hev1': 'H265/HEVC', 'vp9': 'VP9', 'vp09': 'VP9',
                   'vp8': 'VP8', 'mp4a': 'AAC', 'opus': 'Opus'}
    ...
    if contains_audio: parsed['audio'] = {
        'sample_rate': f'{round(format["asr"] / 1000, 2)}khz' if 'asr' in format else 'unknown sample rate',
        'bitrate': f'{round(format["abr"])}kbps' if format.get('abr', None) and float(
            format['abr']) else 'unknown bitrate',
        'codec': parse_codec(format['acodec'])}
    return parsed


def get_video_name():
    info = extract_info(url=url_input.get(), ydl_opts=ydl_base_opts)
    title = info['title']
    if title:
        name_input.delete(0, END)
        name_input.insert(0, title)
...
def handle_download_info(url: str, path: str, ydl_opts: dict = None):
    ...
    details_window = Toplevel(
        takefocus=True)  # no need mainloop here as below we use the general global mainloop function
    details_window.title('Extracted Info')
    Label(details_window, text=f'{parsed_info["title"]} ({parsed_info["duration"]})', anchor=CENTER).pack(side=TOP,
                                                                                                          fill=X,
                                                                                                          expand=True,
                                                                                                          padx=10)
    ...
    def on_select_single_format(*args):
        if video_only_formats[video_selected_format.get()] is None and audio_only_formats[
            audio_selected_format.get()] is None:  # if both selected nothing, unlock radio buttons
            ...
            selected = []
            if video_only_formats[video_selected_format.get()]: selected.append(
                video_only_formats[video_selected_format.get()])
            if audio_only_formats[audio_selected_format.get()]: selected.append(
                audio_only_formats[audio_selected_format.get()])
            selected_format.set('+'.join(selected))  # allow 1 only
        try:
            if video_only_formats[video_selected_format.get()] is None and audio_only_formats[
                audio_selected_format.get()] is not None:
    ...
    def on_select_audio_convert_format(*args):
        audio_convert_quality_selector.configure(
            state=NORMAL if audio_convert_format.get() != 'Do not convert' else DISABLED)

    def handle_download():
        nonlocal ydl_opts
        ydl_opts['format'] = selected_format.get()
        if video_only_formats[video_selected_format.get()] is None and audio_only_formats[
            audio_selected_format.get()] is not None and valid_audio_convert_formats[
            audio_convert_format.get()] is not None:
            ydl_opts['postprocessors'] = [
                {'key': 'FFmpegExtractAudio', 'preferredcodec': valid_audio_convert_formats[audio_convert_format.get()],
                 'preferredquality': audio_convert_quality_values[
                     audio_convert_quality.get()]}]  # 0 highest, 10 lowest.
        ydl_opts['outtmpl'] = os.path.join(path, ydl_opts['outtmpl'] if isinstance(ydl_opts['outtmpl'], str) else
        ydl_opts['outtmpl']['default'])
        download_queue.append(DownloadTask(url, path, ydl_opts, queue_frame))
    ...
    download_button = Button(scrollableFrame.scrollwindow, text='Download',
                             command=handle_download)  # have to define first else callbacks below complain
    selected_format = StringVar()
    selected_format.trace('w', on_select_format)
    video_only_formats, audio_only_formats = {'Select video format': None}, {
        'Select audio format': None}  # {text: value} except for None
    ...
    for f in formats:
        if f['video'] and f['audio']:  # video AND audio
            ...
            if f['video']: video_only_formats[
                f'{f["video"]["resolution"]}@{f["video"]["fps"]}fps {f["video"]["codec"]} {"HDR" if f["video"]["hdr"] else ""}'] = \
                f['format_id']
            if f['audio']: audio_only_formats[
                f'{f["audio"]["sample_rate"]} {f["audio"]["bitrate"]} {f["audio"]["codec"]}'] = f['format_id']
    if not len(radiobutton_frame.winfo_children()): Label(radiobutton_frame,
                                                          text='No stream containing both video and audio available',
                                                          anchor=CENTER).pack(side=TOP, fill=X, expand=True)
    ...
    OptionMenu(custom_formats_frame, video_selected_format, 'Select video format', *video_only_formats.keys()).pack(
        side=TOP, fill=X, expand=True, pady=(0, 10))
    OptionMenu(custom_formats_frame, audio_selected_format, 'Select audio format', *audio_only_formats.keys()).pack(
        side=TOP, fill=X, expand=True, pady=(0, 10))
    audio_convert_format = StringVar(value='Do not convert')
    valid_audio_convert_formats = {'Do not convert': None, 'AAC (.m4a)': 'aac', 'ALAC (.m4a)': 'alac',
                                   'FLAC (.flac)': 'flac', 'm4a (.m4a)': 'm4a', 'mp3 (.mp3)': 'mp3',
                                   'Opus (.opus)': 'opus', 'Vorbis (.ogg)': 'vorbis', 'WAV (.wav)': 'wav'}
    ...
    audio_convert_selector = OptionMenu(audio_convert_frame, audio_convert_format, 'Do not convert',
                                        *valid_audio_convert_formats.keys(), command=on_select_audio_convert_format)
    audio_convert_selector.pack(side=TOP, fill=X, expand=True, pady=(5, 5))
    audio_convert_selector.configure(state=DISABLED)
    audio_convert_quality = StringVar(value='5')
    audio_convert_quality_values = {k: v for v, k in enumerate(
        ['0 (Highest Quality)', '1', '2', '3', '4', '5 (Medium Quality)', '6', '7', '8', '9', '10 (Lowest Quality)'])}
    audio_convert_quality_selector = OptionMenu(audio_convert_frame, audio_convert_quality, '5 (Medium Quality)',
                                                *audio_convert_quality_values)
...
name_input_frame.pack(expand=True, fill=BOTH, side=TOP, pady=(5, 10))
Label(name_input_frame, text='Name: ').pack(side=LEFT, padx=(10, 0))
name_input = Entry(name_input_frame, width=50)
name_input.pack(expand=True, fill=X, side=LEFT, padx=(2, 0))
name_select_button = Button(name_input_frame, text='Get Video Name', command=get_video_name)
name_select_button.pack(side=LEFT, padx=(2, 10))

buttons_frame = Frame(root)
buttons_frame.pack(expand=True, fill=BOTH, side=TOP, anchor=NW)
download_video_button = Button(buttons_frame, text='Download Video',
                               command=lambda: handle_download_video_best(url_input.get(), path_input.get()))
download_video_button.pack(expand=True, fill=X, side=LEFT, padx=(10, 0))
download_audio_button = Button(buttons_frame, text='Download Audio',
                               command=lambda: handle_download_audio_best(url_input.get(), path_input.get()))
download_audio_button.pack(expand=True, fill=X, side=LEFT, padx=(2, 0))
download_info_button = Button(buttons_frame, text='Customize Downloads',
                              command=lambda: handle_download_info(url_input.get(), path_input.get()))
